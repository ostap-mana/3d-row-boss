...
результатом. На перемозі вона висить на банері — `FIGURE_H`, `FIGURE_W`,
`FIGURE_SINK`. На поразці її ставить `standByRetry`, і не від банера, а від
плашки RETRY:

- `POINT.tip` — де в кадрі кінчик пальця: 0.25 ширини від лівого краю, 0.877
  згори. Спрайт зсувається вбік рівно настільки, щоб над плашкою став палець, а
  не щоб над нею стала вона сама.
- `POINT.hem` — 0.387..0.965 ширини: смуга, якою нижній край кадру ріже їй ноги.
  Цей зріз має лишитися за плашкою, тому палець клампиться так, щоб уся смуга
  влізла між її краями. Звідси й `RETRY_W` 0.68: на вужчій плашці зріз не
  ховається, і чобіт стирчить збоку рівною горизонтальною лінією.
- `POINT.overlap` — 0.1 висоти плашки: рівно стільки, щоб зріз пішов під її
  верхній край. Глибше — вона тоне в кнопці, мілкіше — зріз видно.
- `PLATE_Y.defeat` 0.4 і `RETRY_DROP` 0.9 розводять банер і плашку, бо росте
  вона рівно в тому проміжку: `stand` бере мінімум зі стель і з `foot - top`, і
  на телефоні майже завжди виграє проміжок.
